...
#    ]),
  ...
  [
    (set_spawn_radius, 1),
    (store_add, ":p_town_32_plus_one", "p_town_32", 1),
    (store_random_in_range, ":selected_town", "p_town_1", ":p_town_32_plus_one"),
...
#  (14.2, 0, 0.0, [],
#                     [
#                         (assign, "$pin_faction", "fac_kinai_rebels"),
#                         (assign, "$pin_party_template", "pt_kinai_rebels"),
#                         (assign, "$pin_limit", 5),
#                         (call_script,"script_cf_spawn_party_at_faction_town_if_below_limit"),
#                    ]),
...
#Check for political issues
            ...
            (try_begin),
                ...
                (try_begin),
					#gekokujo 3.0 companion interaction point of interest (determines quitting chance from morale loss)
                    (lt, ":npc_morale", 20),
...
#Check for new personality clashes
			...
			(else_try), 
				...
				(else_try),
					...
					(troop_set_slot, "trp_hired_warrior_veteran", slot_troop_mission_object, ":npc"),
					(assign, "$npc_to_rejoin_party", "trp_hired_warrior_veteran"),
				##diplomacy end
				(else_try), 
     ...
     (try_for_range, ":companion", companions_begin, companions_end),
       ...
       (try_for_range, ":i_slot", 0, ":inv_cap"),
		 ...
		 (ge, ":item_id", 0),
	 	 ...
	 	 (this_or_next|eq, ":item_id", "itm_gekokujo_nodachi_1"),
	 	 (this_or_next|eq, ":item_id", "itm_gekokujo_nodachi_2"),
		 (eq, ":item_id", "itm_gekokujo_nodachi_3"),
...
##diplomacy end
  ...
  # (24, 0, 0.0, [], [  ## Set the reinforcements interval to 36 Game hours. Change as you feel like.
    # (try_for_range, ":center", walled_centers_begin, walled_centers_end),
      ...
      # (try_begin),
        ...
        # (lt, ":garrison", 300),            ## Under this number of troops will towns get reinforcements
        # (assign, ":party_template", "pt_reinforcements"),
      # (else_try),
        # (party_slot_eq, ":center", slot_party_type, spt_castle),
        # (lt, ":garrison", 100),            ## Under this number of troops will castles get reinforcements
        # (assign, ":party_template", "pt_reinforcements"),
      # (try_end),
      # (try_begin),
        # (gt, ":party_template", 0),
        # (try_for_range, ":village_reinforcements", villages_begin, villages_end),
          # (try_begin),
			...
			# #gekokujo 3.0 allow player-owned centers to be reinforced as well
